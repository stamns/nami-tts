...
class NanoAITTS:
    ...
    def split_text(self, text, max_chars=500):
        ...
        while i < n:
            ...
            # 寻找最佳分割点
            # 优先在 max_chars 附近的句子结束符分割
            # 搜索范围：从 i + max_chars 向前搜索到 i + max_chars // 2
            search_end = min(i + max_chars, n)
            split_pos = -1
    ...
    def process_long_text(self, text, voice, speed, pitch, volume, language, gender, timeout, retry_count):
        ...
        try:
            with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
                # 提交所有任务
                # 注意：这里递归调用 self.get_audio，因为 chunks 已经是切分好的，长度 < 500
                # 所以 get_audio 不会再次进入 process_long_text
                future_to_index = {
                    executor.submit(
                        self.get_audio, 
